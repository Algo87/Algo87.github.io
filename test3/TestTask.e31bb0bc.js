...
var Popup = /*#__PURE__*/function () {
  ...
  _createClass(Popup, [{
    ...
    value: function focusContol() {
      var nodes = this.$popup.querySelectorAll(_focusElements);
      console.log(nodes);
    ...
    value: function listener(event) {
      if (event.target.dataset.close || event.keyCode === 27 && this.opened === true) {
        this.close();
      }
    }
  }, {
    ...
    value: function destroy() {
      this.destroyed = true; // this.$play.removeEventListener("click", this.open);
...
var img = [_main_bg.default, _btn_img.default];
    ...
    if (isInViewport(lazy[i])) {
      if (lazy[i].getAttribute("data-src")) {
        if (dataLazy === "img") {
          ...
          console.log(lazy[i]);
          console.log(img[i]); // lazy[i].style.backgroundImage = `url("${img[i]}")`;
