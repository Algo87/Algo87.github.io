		...
		var altText=$(this).find('img').attr('alt');
			...
			if(!$('.popup-slider').hasClass('popup-slider-open')){
				...
				$('.production-slider').on(`afterChange`, function(event, slick, currentSlide, nextSlide) {
					...
					console.log($('.production-slider .slick-active').find('img').attr('alt'));
			...
			$('.popup-slider').height($(window).height());

			var heightSlider=$('.popup-slider').height()*0.9;

			$('.production-slider').height(heightSlider);
			$('.production-slide').height(heightSlider - 60);
			if($(window).width()<650){
				$('.production-slide').height(217);
			}
...
function tabsChange(){
		...
		if(!!$('.contacts-rigth-part-title').length){
			...
			$(window).ready(function(){
				// console.log($("#right-select option:first"));
				// console.log($('.select-visible option:first'));
			...
			$('.contacts-rigth-part-item ').click(function(e){
				// console.log($('.contacts-rigth-part-item '));
			...
			if(!!$('#right-select')){
				...
				// console.log($('.contact-visible option:first'));
			...
			contactsLeftPartLink.click(function(e){
				...
				if($(this).parent().hasClass(arrLeftPart[i])){
					...
					if($('.contacts-main').hasClass(arrLeftPart[i])){
						// console.log(arrRegionContacts[i]);
	...
	if(!!$('#contact-map').length){
		...
		ymaps.ready(function () {
			...
			myPlacemark = new ymaps.Placemark(myMap.getCenter(), {
				...
				iconImageHref:'C:/work/PROMIS/app/img/contacts-map.svg',
...
$(document).ready(function() {
	...
	function heightWindow(){
		...
		// $('.production-slide').find('img').height()
	})
		}
	...
	contactMainNavLink.onmouseover=function(){
		...
		for (let i=0; i<levelTwoLi.length; i++){
			...
			levelTwoLi[i].onmouseover=function(){
				// console.log(this.querySelectorAll('a')[0]);
		...
		$('.about-honor').find($('.about-honor-img')).click(function(e){
			...
			$('.about-slider').on(`init reInit`, function(event, slick) {
				console.log('ok');
					...
					// console.log(1+Number(slideAbout));
			...
			$('.about-slider').on(`afterChange`, function(event, slick, currentSlide) {
				console.log(currentSlide);
					...
					// console.log(currentSlide);
					// console.log(slick.slideCount);
				// console.log($('.about-slider .slick-active').find('img').attr('alt'));
